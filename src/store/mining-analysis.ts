...
import { miningTransactionsAtom, miningClaimTransactionsAtom } from "./citycoins";
    ...
    return miningTxs
      ...
      .map(tx => {
        ...
        const claimedStatusArray = blocksToClaimArray.map(blockHeight => {
          const claimTx = findClaimTransaction(blockHeight, tx.sender_address, cityName, claimTxs);
          return {
...
function extractMiningArgs(tx: Transaction): { cityName: string; amounts: number[] } {
  ...
  // Extract city name
  const cityNameArg = tx.contract_call.function_args.find(
    arg => arg.type.includes("string")
  );
...
function findClaimTransaction(
  ...
  return claimTxs.find(tx => {
    ...
    return claimArgs.claimHeight === blockHeight && 
...
function extractClaimArgs(tx: Transaction): { cityName: string; claimHeight: number } {
  if (tx.tx_type !== "contract_call" || !tx.contract_call.function_args) {
  ...
  const heightArg = tx.contract_call.function_args.find(
    arg => arg.type.includes("uint")
  );
